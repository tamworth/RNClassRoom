...
//组件的
const componentsModulesIndexMapFileName = `${__dirname}/modules-index-map-${buildType}.json`;
const componentsModulesIndexMap = require(`${base}/compile/modules-index-map-${buildType}.json`);

function clearFileInfo() {
  if (moduleIdByIndex) {
    fs.writeFileSync(commonModulesIndexMapFileName, '{}');
    fs.writeFileSync(componentsModulesIndexMapFileName, '{}');
  } else {
    fs.writeFileSync(commonModulesFileName, '[]');
  }
}

function createComponentsModuleIdFactory() {
  ...
  return path => {
    ...
    if (typeof id !== 'number') {
      ...
      componentsModulesIndexMap[relPath] = id;
      fs.writeFileSync(componentsModulesIndexMapFileName, JSON.stringify(componentsModulesIndexMap));
      console.log("fileToIdMap false", relPath, id);
    }
    // console.log("===path3:", path, id);
    return id;
  };
}

function processComponentsModuleFilter() {
  return module => {
    const {path} = module;
    const relPath = pathM.relative(base, path);
    if (
      ...
      // console.log("===processModuleFilter false1:", path);
      return false;
    }
    // 使用name的情况
    if (!moduleIdByIndex) {
      if (commonModules.includes(relPath)) {
        // console.log("===processModuleFilter false2:", relPath);
        return false;
      }
    } else {
      // 使用id的情况
      if (commonModulesIndexMap[relPath]) {
        // console.log("===processModuleFilter false3:", relPath);
        return false;
      }
    }
    // console.log("===processModuleFilter success:", relPath);
...
function createModuleIdFactory() {
  ...
  return path => {
    ...
    if (componentsModulesIndexMap[relPath]) {
      let idx = componentsModulesIndexMap[relPath];
      // console.log("===path2:", relPath, idx);
      return idx;
    }
...
function processModuleFilter() {
  return module => {
    ...
    if (componentsModulesIndexMap[relPath]) {
      console.log("===processModuleFilter false4:", relPath);
      return false;
    }
